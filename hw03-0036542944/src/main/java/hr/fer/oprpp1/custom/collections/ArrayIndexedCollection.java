...
public class ArrayIndexedCollection<T> implements List<T> {
	...
	public void insert(T value, int position) {
		...
		if (position != this.size) {
			T[] temp = Arrays.copyOfRange(this.elements, position, this.size);
	...
	private static class ArrayIndexedElementsGetter<T> implements ElementsGetter<T>{
		...
		/**
		 * Index of next element in <code>ElementsGetter</code>
		 * @since 1.0.0.
		 */
		...
		public boolean hasNextElement() {
			...
			return this.currentElementIndex < collection.size;
		}
		...
		public T getNextElement() {
			if(this.hasNextElement()) {
				...
				return collection.elements[currentElementIndex - 1];
			}
